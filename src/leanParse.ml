...
module RRange : sig
  ...
  let append { chunks; tail; len } x =
    ...
    if len mod chunk_size <> 0 then { chunks; tail; len }
      ...
      (* [Range] is newest-first; chunks are in index order. *)
      let chunk =
...
(* Preserve the pre-chunking Marshal layout. *)
...
module Checkpoint = struct
  (* Encode DAG edges as indices to avoid Marshal's object-identity table. *)
  type 'a node =
  ...
  let find_slot index value =
    ...
    let rec probe slot =
      ...
      if stored = 0 || index.values.(stored - 1) == value then slot
      else probe (if slot + 1 = capacity then 0 else slot + 1)
    in
    probe (hash value mod capacity)

  let make_index range =
    let length = RRange.length range in
    if length > Sys.max_array_length / 2 then invalid ();
    (* Power-of-two rounding could double this temporary allocation. *)
